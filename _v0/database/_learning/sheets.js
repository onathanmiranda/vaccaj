...
let sheetsCSV = 'id,image_files';
let sheetVoiceRelationCSV = 'sheet_id,voice_type_id';

sheets.forEach((sheet, index) => {
  let _id = index + 1;

  sheetsCSV += `\n${_id},"[${sheet.filePaths.reduce((prev, curr, index) => {
    return `${prev}${index > 0 ? ',' : ''}{""path"": ""${curr}"", ""order"": ${index}}`
  }, '')}]"`;

  sheet.voiceTypes.forEach((voiceType) => {
    switch(voiceType) {
      case '0':
        sheetVoiceRelationCSV += `\n${_id},1\n${_id},2\n${_id},3\n${_id},4\n${_id},5\n${_id},6`
        break;
      case '7':
        sheetVoiceRelationCSV += `\n${_id},6\n${_id},5\n${_id},3`
        break;
...
fs.writeFileSync('sheets.csv', sheetsCSV);
fs.writeFileSync('sheets_voice_types.csv', sheetVoiceRelationCSV);
