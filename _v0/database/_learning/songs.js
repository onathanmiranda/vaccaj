...
const fs = require('fs');
let songsCSV = 'id,title,slug,beginning,instructions,lyrics';

let songsRecordingsCSV = 'song_id,recording_id';
let songsSheetsCSV = 'song_id,sheet_id';

const recordingsOriginal = require('./recordings');

songs.forEach((song, index) => {
  let _id = index + 1;
  songsCSV += `\n${_id},"${song.title}","${song.slug}",${song.beginning || null},${song.instructions ? '"'+song.instructions.replace(/(?:\r\n|\r|\n)/g, '\\n').replaceAll('"', '""') + '"' : null},${song.lyrics ? '"' + song.lyrics.replace(/(?:\r\n|\r|\n)/g, '\\n') + '"' : null}`;

  song.recordingIds.forEach((recordingId) => {
    const recording_id = recordingsOriginal.findIndex(recording => recording.id === recordingId);
    if(recording_id === -1) throw new Error(`Recording not found: ${recordingId}`);
    songsRecordingsCSV += `\n${_id},${recording_id + 1}`;
  });

  song.sheetsList.forEach((sheetId) => {
    songsSheetsCSV += `\n${_id},${sheetId}`;
  });
});

fs.writeFileSync('songs.csv', songsCSV);
fs.writeFileSync('songs_recordings.csv', songsRecordingsCSV);
fs.writeFileSync('songs_sheets.csv', songsSheetsCSV);
